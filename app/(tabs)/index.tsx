...
const Dashboard = () => {
  ...
  // 4. Savings
  // useMemo so it only recalculates if the 'savings' array changes
  ...
  const totalBalance =
    weeklyIncome - weeklySpending - billsPaidThisWeek - totalCurrentSavings;
  ...
  const StatCard = ({ label, value, icon, color, borderColor }: any) => (
    <View
      className={`w-36 h-36 mr-3 p-4 rounded-[24px] justify-between bg-[#1a1a1a] border ${borderColor}`}
    >
      <View
        className={`w-10 h-10 rounded-full items-center justify-center bg-${color}-500/10`}
      >
        <Ionicons
          name={icon}
          size={20}
      ...
      <View>
        <Text className="text-gray-500 text-[10px] uppercase font-rubik-medium tracking-widest mb-1">
          {label}
        </Text>
        <Text
          className={`text-xl font-rubik-bold ${color === "white" ? "text-white" : `text-${color}-500`}`}
        >
          {value}
        </Text>
      </View>
  ...
  const ActionButton = ({ icon, label, onPress, color }: any) => (
    <TouchableOpacity onPress={onPress} className="items-center gap-2">
      <View
        className={`w-14 h-14 bg-[#1a1a1a] rounded-full items-center justify-center border border-white/10`}
      >
        <Ionicons name={icon} size={24} color={color} />
      </View>
      <Text className="text-gray-400 text-[10px] font-rubik uppercase tracking-wide">
        {label}
      </Text>
    </TouchableOpacity>
  );
        ...
        <View className="px-6 mb-8">
          <View className="bg-[#1a1a1a] rounded-[32px] p-7 border border-white/5 relative overflow-hidden">
            ...
            <View className="flex-row items-baseline mt-2">
              <Text
                className={`text-6xl font-rubik-bold ${totalBalance < 0 ? "text-red-500" : "text-white"}`}
              >
          ...
          <View className="px-6 mb-8">
            <View className="flex-row justify-between items-center mb-4">
              ...
              <TouchableOpacity onPress={() => router.push("/savings")}>
                <Text className="text-teal-500 text-xs font-rubik">
                  See All
                </Text>
              </TouchableOpacity>
            </View>
            ...
            {savings.slice(0, 3).map((goal) => {
              ...
              return (
                <View
                  key={goal.id}
                  className="bg-[#1a1a1a] rounded-2xl p-4 mb-3 border border-white/5"
                >
        ...
        <View className="px-6 mb-6">
          ...
          {transactionsThisWeek.length === 0 ? (
            ...
            transactionsThisWeek.slice(0, 5).map((t) => (
              <View
                key={t.id}
                className="bg-[#1a1a1a] rounded-2xl p-4 mb-3 border border-white/5 flex-row justify-between items-center"
              >
          ...
          <View className="px-6 pb-20">
            <View className="flex-row justify-between items-center mb-4">
              ...
              <TouchableOpacity onPress={() => router.push("/bill")}>
                <Text className="text-blue-500 text-xs font-rubik">
                  See All
                </Text>
              </TouchableOpacity>
            </View>
